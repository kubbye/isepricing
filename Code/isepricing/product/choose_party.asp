...
<script language="javascript">
    ...
    document.form1.action="choose_party.asp?<%=URLCS%>&thispage="+str;
...
function confirmparty(cid, itemcode, itemname, unitcost, unitcostrmb){
	//if(confirm("Your select Item is " + itemname + " ,confirm it?")){
		...
		c3.innerHTML = "<input type='hidden' name='<%=tableid%>_cid" + count + "' value='" + cid + "'><input type='hidden' name='<%=tableid%>_itemcode" + count + "' value='" + itemcode + "'>" + itemcode;

		c4 = newRow.insertCell(3);
		c4.innerHTML = "<input type='hidden' name='<%=tableid%>_itemname" + count + "' value='" + itemname + "'>" + itemname;

		c5 = newRow.insertCell(4);
		c5.innerHTML =  "<input type='text' name='<%=tableid%>_qty" + count + "' value='1' size='3' onblur='validPlusIntParty(this, \"<%=tableid%>\");'>";
		
		c6 = newRow.insertCell(5);
		c6.innerHTML =  "<input type='hidden' name='<%=tableid%>_unitcost" + count + "' value='" + unitcost + "'><input type='hidden' name='<%=tableid%>_unitcostrmb" + count + "' value='" + unitcostrmb + "'>" + unitcost;
		
		c7 = newRow.insertCell(6);
		c7.innerHTML =  "<div id='<%=tableid%>_quotedcost" + count + "'>" + unitcost + "</div>";
      ...
      <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
          ...
          <tr> 
            <td valign="top" class="line01"> <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
                <tr> 
                  <td>
				    <form name="form1" action="choose_party.asp" method="post">
