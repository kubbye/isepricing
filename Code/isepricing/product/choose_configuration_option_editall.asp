...
function PageTo(str) {
    ...
    document.form1.action="choose_configuration_option_editall.asp?<%=URLCS%>&thispage="+str;
 ...
 function confirmconfiguration(cid, materialno, description, listprice){
	if(confirm("Your select Material No is " + materialno + " ,confirm it?")){
		...
		c3.innerHTML = "<input type='text' name='<%=tableid%>_items" + count + "'>";
		...
		c7.innerHTML =  "<div id='<%=tableid%>_targetpricediv" + count + "'>&nbsp" + listprice + "</div>";
		
		c8 = newRow.insertCell(7);
		c8.innerHTML =  "<input type='text' size='6' name='<%=tableid%>_discount" + count + "' value='1' onblur='validPlusNumeric(this, \"<%=tableid%>\", \"" + count + "\", \"" + listprice + "\")'>";
      ...
      <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
          ...
          <tr> 
            <td valign="top" class="line01"> <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
                <tr> 
                  <td>
				    <form name="form1" action="choose_configuration_option_editall.asp" method="post">
