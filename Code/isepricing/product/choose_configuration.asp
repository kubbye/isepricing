...
function PageTo(str) {
    ...
    document.form1.action="choose_configuration.asp?<%=URLCS%>&thispage="+str;
 ...
 function confirmconfiguration(cid, materialno, description, listprice){
	//if(confirm("Your select Material No is " + materialno + " ,confirm it?")){
		...
		c2.innerHTML = "<select name='<%=tableid%>_mutex" + count + "'><option value=''></option><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option><option value='8'>8</option><option value='9'>9</option><option value='10'>10</option><option value='11'>11</option><option value='12'>12</option><option value='13'>13</option><option value='14'>14</option><option value='15'>15</option></select>";

		c3 =  newRow.insertCell(2);
		c3.innerHTML = "<input type='text' name='<%=tableid%>_sortno" + count + "' size='3' onblur='validPlusInt2(this, \"<%=tableid%>\");'>";

		c4 = newRow.insertCell(3);
		c4.innerHTML = "<input type='hidden' name='<%=tableid%>_cid" + count + "' value='" + cid + "'><input type='hidden' name='<%=tableid%>_materialno" + count + "' value='" + materialno + "'>" + materialno;

		c5 = newRow.insertCell(4);
		c5.innerHTML =  "<input type='hidden' name='<%=tableid%>_description" + count + "' value='" + description + "'>" + description;

		c6 = newRow.insertCell(5);
		c6.innerHTML ="<input type='text' name='<%=tableid%>_qty" + count + "' value='1' size='3' onblur='validPlusInt(this, \"<%=tableid%>\");'>";
		
		c7 = newRow.insertCell(6);
		c7.innerHTML =  "<input type='hidden' name='<%=tableid%>_listprice" + count + "' value='" + parseFloat(listprice).toFixed(0) + "'>" + parseFloat(listprice).toFixed(0);
		
		c8 = newRow.insertCell(7);
		c8.innerHTML = "<div id='<%=tableid%>_totallistpricediv" + count + "'>" + parseFloat(listprice).toFixed(0) + "</div>";
      ...
      <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
          ...
          <tr> 
            <td valign="top" class="line01"> <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
                <tr> 
                  <td>
				    <form name="form1" action="choose_configuration.asp" method="post">
