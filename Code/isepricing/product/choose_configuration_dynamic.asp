...
function PageTo(str) {
    ...
    document.form1.action="choose_configuration_dynamic.asp?<%=URLCS%>&thispage="+str;
    document.form1.submit();
 }

 function confirmconfiguration(materialno){
	//if(confirm("Your select Material No is " + materialno + " ,confirm it?")){
	    alert ("Selected " + materialno);
		window.opener.document.all("options").value = materialno;
		window.close();
	//}
 }
</script>
      ...
      <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
          ...
          <tr> 
            <td valign="top" class="line01"> <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
                <tr> 
                  <td>
				    <form name="form1" action="choose_configuration_dynamic.asp" method="post">
          ...
          <tr> 
            <td><table width="100%" border="1" bordercolordark="#FFFFFF" bordercolorlight="#CCCCCC" cellspacing="0" cellpadding="0" >
				...
				%>
					 <tr class="line02"> 
					  <td><input type="radio" value="on" onclick="confirmconfiguration('<%=rs("materialno")%>')"></td>
					  <td><%=rs("materialno")%></td>
