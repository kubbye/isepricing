...
<%
	...
	If Not rs_statusQuery.eof And Not rs_statusQuery.bof Then
		response.write("<script>alert('请先“Delete”本单的特价申请，再Close本单!');</script>")
	...
	Set rs_statusQuery=conn.execute(sql)
	If Not rs_statusQuery.eof And Not rs_statusQuery.bof Then
		response.write("<script>alert('请先处理该单的SpecialPrice再“Close”!');</script>")
		response.write("<script>location.href='quotationList.asp';</script>")
		response.end
	End If 

	strsql="update quotation set status=13,updtuser='"&session("principleid")&"',updttime='"&getdate()&"' where qid="&qid
	conn.execute strsql
	'发送邮件
