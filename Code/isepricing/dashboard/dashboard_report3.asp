      ...
      <td><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0" class="table01">
          ...
          <tr> 
            <td><table width="100%" border="1" bordercolordark="#FFFFFF" bordercolorlight="#CCCCCC" cellspacing="0" cellpadding="0" >
				...
				%>
					<tr class="line02">
					  ...
					  If zone <> "" And region = "" Then 
						sql = "select vquarter, sum(targetprice) target, sum(netoit) oit, sum(qty) qty from dashboard where oacm='" & zone & "' and productname='" & rs("productname") & "' and vyear='" & vyear & "'"
					  ElseIf zone = "" And region <> "" Then
						sql = "select vquarter, sum(targetprice) target, sum(netoit) oit, sum(qty) qty from dashboard where oacm in (select zonename from zone where region='" & region & "') and productname='" & rs("productname") & "' and vyear='" & vyear & "'"
					  ElseIf zone = "" And region = "" Then 
					  ...
					  <td><%If zone <> "" Then %><%=zone%><% ElseIf region <> "" Then %><%=region%><% Else %>Greater China<% End If %></td>
					  <%
					  If vquarter = "Q4" Then
					  %>
					  <td><%=Round(target4)%></td>
				...
				'计算BU的合计数据
				If zone <> "" And region = "" Then 
					sql = "select vquarter, sum(targetprice) target, sum(netoit) oit, sum(qty) qty from dashboard where oacm='" & zone & "' and bl='" & bu & "' and vyear='" & vyear & "'"
				ElseIf zone = "" And region <> "" Then
					sql = "select vquarter, sum(targetprice) target, sum(netoit) oit, sum(qty) qty from dashboard where oacm in (select zonename from zone where region='" & region & "') and bl='" & bu & "' and vyear='" & vyear & "'"
			    ElseIf zone = "" And region = "" Then 
			  ...
			  %>
				<tr class="line02" bgcolor="#E1FFFF">
				  <td><a href="javascript:intoproduct('')"><%=bu%></a></td>
				  <td>Subtotal</td>
				  <%
				  If vquarter = "Q4" Then
				  %>
