            ...
            <td><table width="100%" border="1" bordercolordark="#FFFFFF" bordercolorlight="#CCCCCC" cellspacing="0" cellpadding="0" >
				...
				While Not rs.eof
					...
					While Not rs1.eof
						...
						Wend 
							  %>
							<tr class="line02">
							  <td><a href="javascript:intozonebu('<%=rs1("zonename")%>')"><%=rs1("zonename")%></a></td>
							  <td><%If bu <> "" Then %><%=bu%><% Else %>IS<% End If %></td>
							  <%
							  If vquarter = "Q4" Then
							  %>
							  <td><%=Round(target4)%></td>
				...
				<%
					...
					Wend 
						...
						<tr class="line02" bgcolor="#E1FFFF">
						  <td><a href="javascript:intoregionbu('<%=rs("region")%>')"><%=rs("region")%></a></td>
						  <td><%If bu <> "" Then %><%=bu%><% Else %>IS<% End If %></td>
						  <%
						  If vquarter = "Q4" Then
						  %>
				...
				If region = "" Then 
					...
					<tr class="line02" bgcolor="#8DB4E3">
					  <td><a href="javascript:intototalbu()">Subtotal</a></td>
					  <td><%If bu <> "" Then %><%=bu%><% Else %>IS<% End If %></td>
					  <%
					  If vquarter = "Q4" Then
					  %>
