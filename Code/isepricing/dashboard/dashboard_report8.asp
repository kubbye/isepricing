...
function intozoneproduct(zone, product){
	document.form1.bu.value = product;
            ...
            <td><table width="100%" border="1" bordercolordark="#FFFFFF" bordercolorlight="#CCCCCC" cellspacing="0" cellpadding="0" >
				...
				While Not rs.eof
					...
					While Not rs1.eof
						...
						sql = "select distinct bl from dashboard where 1=1"
						If bu <> "" Then
							sql = sql & " and bl='" & bu & "'"
						End If 
						...
						While Not rsproduct.eof
							...
							sql = "select vquarter, sum(targetprice) target, sum(netoit) oit, sum(qty) qty from dashboard where oacm='" & rs1("zonename") & "' and vyear='" & vyear & "' and bl='" & rsproduct("bl") & "'"
							...
							Wend 
							  %>
							<tr class="line02">
							  <td><a href="javascript:intozoneproduct('<%=rs1("zonename")%>', '<%=rsproduct("bl")%>')"><%=rs1("zonename")%></a></td>
							  <td><%=rsproduct("bl")%></td>
							  <%
							  If vquarter = "Q4" Then
							  %>
